 ...
 .. list-table::
   ...
   * - | (7)
     - | bodyを定義しているjspファイルの置き場所について、{1}にリクエストパス、{2}にJSP名が一致するように設計する。
       | これにより、Controllerから返却されるView名ごとの定義を記述する手間を省くことができる。
...
Controllerから返却されたView名が ``<contextPath>/staff/create?form`` の場合、
...
- tiles-definitions.xml
 ...
 .. list-table::
   ...
   * - | (2)
     - | 今回追加するレイアウトについて、描画の際にControllerから返却されたView名がnameのパターンと同じ場合に呼ばれるレイアウト定義。
       | そのView名が<contextPath>/\*/search\*に該当する場合、このレイアウト定義が読み込まれる。
       | extendsしている レイアウト定義"layoutsOfSearch"も適用される。
   * - | (3)
     - | 今回追加するレイアウトで使用するタイトルを指定する。
       | valueはspring-mvcに取り込まれているpropertiesの中から取得する。(以下の説明では application-messages.propertiesに設定する。)
       | {1}はControllerから返却されるView名の"\*/search\*"の「*」の1つ目。
       | {2}はControllerから返却されるView名の"\*/search\*"の"search*"に該当する為、先頭が"search"で始まる必要がある。
   * - | (4)
 ...
 .. note::
     ...
     上記の場合、スタッフ検索画面へのControllerから返却されるView名が複数パターンに該当するため、1番上にレイアウト定義している。
...
Controllerから返却されるView名が ``<contextPath>/staff/search`` の場合、
以下のように別のレイアウトを構築して画面描画を行う。
 ...
 .. list-table::
   ...
   * - | (5)
     - | Controllerから返却されたView名が<contextPath>/\*/search\*に該当する場合、このレイアウト定義が読み込まれる。
